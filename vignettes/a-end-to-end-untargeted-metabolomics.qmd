...
defined. The primary objective of preprocessing is the quantification of signals
from ions measured in a sample, addressing any potential retention time drifts
...
a possible evalaution is to which features are overlapping between the different
sample groups.

Fisrt we can plot the overlapping between the CVD and CTR group.

Before performing complex satistical analysis we can evaluate the data

```{r}
res_df <- cbind(CVD = rowSums(is.na(assay(res, "raw_filled"))) < 10,
                 CTR = rowSums(is.na(assay(res, "raw_filled"))) < 10)
...
below we compare the overlap between samples.
